...
class Assignment(db.Model):
    ...
    @classmethod
    def gradeAssignment(cls, assignment_new: 'Assignment'):
        if assignment_new.id is not None:
            assignment = Assignment.get_by_id(assignment_new.id)
            assertions.assert_found(
                assignment, 'No assignment with this id was found')
            ...
            assertions.assert_valid(assignment_new.grade is not None,
                                    'assignment grade cannot be null')
            assertions.assert_valid(GradeEnum.has_value(assignment_new.grade),
                                    'assignment grade must be one of A, B, C, D')
            assertions.assert_valid(assignment.teacher_id == assignment_new.teacher_id,
                                    'assignment must be assigned to the same teacher')
